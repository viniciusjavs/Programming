...
/*
  ...
  and the values, stored in a vector, sorted in increasing order.
...
int main()
{
    ...
    while (cin >> value >> unit) {
        ...
        if (unit == "cm" || unit == "m" || unit == "in" || unit == "ft") {
            ++counter;
            cout << '\'' << value << '\'' << " '" << unit << "'\n";
            if (value_in_m < smallest) smallest = value_in_m;
            if (value_in_m > largest) largest = value_in_m;
    ...
    for (double value : values) cout << value << ' ';
    cout << '\n';
}
