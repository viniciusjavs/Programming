...
/*
  ...
  *It can be a font of problems, because it can't be possible to know in advance
  if a variable name is used to be assigned or dereferenced.
  ...
  Provide some way of help it's user
  Change the q and h commands to be quit and help
 */
...
class Token {
  public:
    ...
    Token(char ch, double val) : kind{ch}, value{val} {} // initialize kind and
                                                         // value
...
constexpr char cnst = 'C';   // constant token
...
constexpr char help = 'H'; // help token
constexpr char number = '8';
constexpr char name = 'a';      // name token
const string declkey = "let";   // declaration keyword
const string cnstkey = "const"; // constant keyword
const string sqrtkey = "sqrt";  // sqrt keyword
const string powerkey = "pow";  // pow keyword
const string quit_s = "quit";   // quit keyword
const string help_s = "help";   // help keyword
 ...
 */
{
    ...
    char ch;
    while (cin.get(ch)) { // doesn't skip whitespace (sapce, newline, tab, etc)
	switch (ch) {
	    //case quit:
	case print:
	    //case help:
	...
	default:
		...
		if (s == cnstkey) return Token{cnst}; // constant keyword
		if (s == sqrtkey) return Token{root}; // sqrt keyword
		if (s == powerkey) return Token{power}; // pow keyword
		if (s == quit_s) return Token{quit}; // quit keyword
		if (s == help_s) return Token{help}; // help keyword 
		return Token{name, s}; // variable name
	    }
	    if (!isspace(ch)) error("Bad token");
	    if (ch == '\n') return Token{print};
	}
    }
    error("EOF");
...
Symbol_table vars; // variables & constants table
 ...
 */
{
    for (Variable &v : var_table)
        if (v.name == s) {
	    if (v.is_const) error("set: constant variable ", s);
	    v.value = d;
            return;
        }
 ...
 */
{
    cout << "Be-bugged Calculator\n"
            "To use this calculator you simplest enter an math expression\n"
            "followed by ';' or 'newline'\n"
            "Enter exit or \'Q\' to quit the program\n"
            "e.g.: 3+(45/3)/5;\n";
}
 ...
 */
{
    ...
    Token t2 = ts.get();
    if (t2.kind != assign) error("= missing in declaration of ", var_name);
    
    double d = expression();
    vars.define_name(var_name, d, is_const);
    return d;
}
...
{
    Token t = ts.get();
    if (t.kind != name) error("undeclared variable");
    string var_name = t.name;

    Token t2 = ts.get();
    if (t2.kind != assign) error("= missing in assignment of ", var_name);
