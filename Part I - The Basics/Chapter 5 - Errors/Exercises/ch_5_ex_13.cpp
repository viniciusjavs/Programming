...
*/
{
    ...
    int digit;
    vector<int> guess;
    if (number > pow(10,size) - 1 || number < pow(10,size - 2)) // [100..9999] are valid
...
int main()
{
	...
	char ans = 1;
	while (ans) {
	    cout << "Please enter a integer \"seed\" to generate game randomness: "
                "\n";
	    int seed;
	    cin >> seed;
	    if (!cin)
	    ...
	    vector<int> guess = int_to_v(number);
	    int bulls = 0, cows = 0;
